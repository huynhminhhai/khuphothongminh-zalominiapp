...
import { useGetNotificationDetail, useUpdateNotification } from "apiRequest/notification"
...
const NotificationUpdateForm: React.FC = () => {
    ...
    const { mutateAsync: updateNotification, isPending } = useUpdateNotification();
    const { data: notificationDetail, isLoading } = useGetNotificationDetail(Number(transactionsId));
    ...
    useEffect(() => {
        if (notificationDetail) {
            ...
            reset({ ...notificationDetail })

        }
    }, [notificationDetail, reset])
    ...
    const handleConfirm = async () => {
        ...
        if (formData) {
            try {
                ...
                await updateNotification(dataSubmit);
            } catch (error) {
    ...
    return (
        <Box p={4}>
                ...
                <div className="grid grid-cols-12 gap-x-3">
                    ...
                    <div className="fixed bottom-0 left-0 flex justify-center w-[100%] bg-white box-shadow-3">
                        <Box py={3} className="w-[100%]" flex alignItems="center" justifyContent="center">
                            <PrimaryButton disabled={isPending} fullWidth label={isPending ? "Đang xử lý..." : "Cập nhật thông báo"} handleClick={handleSubmit(onSubmit)} />
                        </Box>
                    </div>
                </div>
            </Box>
            <ConfirmModal
                ...
                title="Xác nhận"
                message="Bạn có chắc chắn muốn cập nhật thông tin thông báo này không?"
