...
import { useGetFeedback1022Detail, useGetFeedback1022File } from "apiRequest/feedback1022";
...
const Feedback1022DetailPage: React.FC = () => {
    ...
    const { data, isLoading } = useGetFeedback1022Detail(feedbackId as string);
    const { data: dataFile, isLoading: isLoadingFile } = useGetFeedback1022File('6827fc212b5c471e5494e29c');
    ...
    return (
        <Page className="relative flex-1 flex flex-col bg-white pb-[30px]">
            <Box>
                <HeaderSub title="Chi tiết phản ánh (từ 1022)" />
                {
                    isLoading ?
                                        ...
                                        <Swiper autoplay duration={8000} style={{ borderRadius: 0 }}>
                                            {imageFiles.map((item, index) => (
                                                <Swiper.Slide key={index}>
                                                    <img
                                                        onClick={() => openUrlInWebview(getFullImageUrl(item.tenTapTin))}
                                                        className="slide-img object-cover w-full h-full"
                                                        src={getFullImageUrl(item.tenTapTin)}
                                                    ...
                                                    <div key={index} className="flex items-center gap-2 justify-between mb-2">
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-1">
                                                                ...
                                                                <div className="text-[14px] font-semibold w-full">{item.tenTapTin}</div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
                                        ...
                                        <Box pt={4} px={4} className="relative">
                                            <Box pb={2} mb={3} className="border-b-[1px]">
                                                <h3 className="text-[18px] leading-[26px] font-semibold">
                                                    Trung tâm điều hành 1022 đã trả lời:
                                                </h3>
...
export default Feedback1022DetailPage